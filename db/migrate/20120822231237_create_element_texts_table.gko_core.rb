...
class CreateElementTextsTable < ActiveRecord::Migration
  def up
    ...
    create_table :element_assignments do |t|
      ...
      t.string :element_type, :null => false, :limit => 40
      t.timestamps
    end
    
    add_index :element_assignments, [:content_id]
    add_index :element_assignments, [:element_id, :element_type]
  end
  
  def down
    remove_index :element_assignments, [:content_id]
    remove_index :element_assignments, [:element_id, :element_type]
